...
export default function BlogPage() {
  ...
  const categories = useMemo(() => {
    ...
    const dynamicCategories = Array.from(articleCategories).map(category => ({
    ...
    () =>
      articles.filter((article) => {
        const matchesSearch =
          ...
          (article.excerpt?.toLowerCase() || '').includes(
  ...
  const otherArticles = useMemo(
    () => filteredArticles.filter((article) => article !== featuredArticle),
  ...
  if (error) {
    ...
    return (
      <main className="min-h-screen bg-white dark:bg-slate-900">
        <Navbar />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center min-h-[60vh] flex items-center justify-center">
          <MotionDiv
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="bg-red-50 dark:bg-red-900/20 p-6 rounded-lg w-full max-w-2xl"
          >
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-900">
      ...
      {/* Hero Section */}
      <MotionSection 
        className="pt-24 pb-16 bg-gradient-to-br from-primary-50 to-white dark:from-slate-800 dark:to-slate-900"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {/* Featured Post */}
      <MotionSection 
        className="py-8 lg:py-12 bg-gray-50 dark:bg-slate-800"
        initial={{ opacity: 0, y: 30 }}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-slate-400">
                    ...
                    <div className="flex items-center">
                      ...
                      {featuredArticle?.createdAt ? new Date(featuredArticle.createdAt).toLocaleDateString(locale) : t('meta.unknownDate')}
                    </div>
                    <div className="flex items-center">
      ...
      {/* Blog Posts Grid */}
      <MotionSection 
        className="py-16 lg:py-20 bg-white dark:bg-slate-900"
        initial={{ opacity: 0 }}
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500 dark:text-slate-400 mb-4">
                    ...
                    <div className="flex items-center">
                      <Calendar className="w-4 h-4 mr-1" />
                      {post.createdAt ? new Date(post.createdAt).toLocaleDateString(locale) : t('meta.unknownDate')}
                    </div>
                  </div>
      ...
      {/* Newsletter Section */}
      <MotionSection 
        className="py-16 lg:py-20 bg-primary-600 dark:bg-blue-600 text-white"
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
      >
            ...
            <div className="max-w-lg mx-auto">
              <form className="flex flex-col sm:flex-row gap-4">
                ...
                <Button 
                  type="submit"
                  variant="secondary" 
                  className="bg-white dark:bg-slate-800 text-primary-600 dark:text-blue-400 hover:bg-gray-50 dark:hover:bg-slate-700 whitespace-nowrap"
                >
                  S&apos;abonner
                </Button>
              </form>
              <p className="text-sm text-primary-100/80 dark:text-blue-200/80 mt-3">
